...
import { STORAGE_DIRS } from '../storage/index.js';
...
function applyDynamicMetadata(manifest: any, folder: string): void {
  ...
  if (manifest.id === 'lily' && (scriptFile.startsWith('models/') || scriptFile.startsWith('dist/'))) {
    // Attempt resolving relative to target
  ...
  if (fs.existsSync(scriptPath)) {
    try {
      if (req.cache) {
        delete req.cache[req.resolve(scriptPath)];
      }
      ...
      console.error('[partner:dynamic-metadata] Error loading script ' + scriptPath, e);
    }
  }
}
...
export function listPartners(userData: string): Record<string, unknown>[] {
  ...
  // In core, we resolve the built-in Lily directory
  // We can pass process.cwd() or similar, or check if dist/models exists.
  const lilyFolder = path.join(userData, 'lily');
