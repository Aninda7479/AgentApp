...
import { MessageSquare, Clock, Plug, Server, Settings, LucideIcon } from 'lucide-react';

/** Destination tabs shown in the mobile bottom navigation bar. */
export type BottomNavTab = 'trajectory' | 'scheduled' | 'plugins' | 'mcp' | 'settings';
        ...
        return (
          ...
          >
            <span className="relative">
              <Icon size={20} />
              {id === 'mcp' && mcpCount > 0 && (
                <span className="absolute -top-1 -right-1.5 min-w-[14px] h-[14px] px-1 rounded-full bg-violet-500 text-white text-[8px] font-bold flex items-center justify-center">
                  {mcpCount}
                </span>
              )}
            </span>
