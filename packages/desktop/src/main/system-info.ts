 ...
 *
 * Runs in the Electron main process (not the renderer) so all platform-specific
 * probing is centralised in one place and the renderer just calls the
 * `system-info` IPC handler. Built on `systeminformation`, which abstracts the
 * per-OS wmi / sysctl / /proc probes.
 *
 * Every probe is wrapped so a single failure degrades gracefully to `null`
 * rather than rejecting the whole IPC call — a user on an unusual machine
 * should still see the hardware we *could* read.
 */
...
export interface SystemInfo {
   ...
   * discrete GPU's VRAM (you can't cheaply span multiple GPUs for one model).
   */
  vramBudgetGB: number;
  /**
