/**
 * Hardware detection for the Local Model (Ollama) manager — web/VPS build.
 *
 * Mirrors the desktop implementation in
 * `packages/desktop/src/main/system-info.ts` so the shared renderer (which is
 * bundled into both shells) gets the same `SystemInfo` shape regardless of
 * where it runs. Runs in the Node server process (not the browser) so the
 * `systeminformation` probes run server-side; the renderer just calls the
 * `system-info` IPC channel over HTTP.
 *
