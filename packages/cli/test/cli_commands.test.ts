...
describe('CLI Wired Slash Command Router (Step 074)', () => {
  it('should register every built-in command from the docs in a single router', () => {
    ...
    expect(names).toEqual(['agent', 'attach', 'btw', 'bug', 'clear', 'compact', 'config', 'cost', 'diff', 'doctor', 'exec', 'export', 'goal', 'help', 'init', 'last', 'learn', 'loop', 'loop-x', 'mcp', 'memory', 'model', 'permissions', 'plan', 'review', 'security-review', 'side', 'status', 'tasks', 'theme', 'verify', 'voice']);
  });
