...
describe('CLI Wired Slash Command Router (Step 074)', () => {
  it('should register every built-in command from the docs in a single router', () => {
    ...
    expect(names).toEqual(['agent', 'attach', 'btw', 'bug', 'clear', 'compact', 'config', 'cost', 'diff', 'doctor', 'exec', 'export', 'goal', 'help', 'init', 'last', 'learn', 'loop', 'loop-x', 'mcp', 'memory', 'model', 'permissions', 'plan', 'review', 'security-review', 'side', 'startup', 'status', 'tasks', 'theme', 'verify', 'voice']);
  });
  ...
  it('should render a help listing of every registered command', () => {
    ...
    for (const name of ['model', 'status', 'theme', 'learn', 'init', 'compact', 'diff', 'doctor', 'permissions', 'btw', 'verify', 'plan', 'review', 'clear', 'tasks', 'export', 'last']) {
  ...
  it('should re-display the last assistant message via /last', async () => {
    const session = createSessionContext();
    let ctxMessages: ContextMessage[] = [
      { role: 'system', content: 'System prompt' },
      { role: 'user', content: 'Hello there' },
    ...
    const res = await router.execute('/last');
    expect(res.success).toBe(true);
    expect(res.output).toContain('**Last Assistant message:**');
    expect(res.output).toContain('Hi! How can I help?');

    // /recall alias should behave identically.
    const aliasRes = await router.execute('/recall');
    expect(aliasRes.success).toBe(true);
    expect(aliasRes.output).toContain('Hi! How can I help?');
  });

  it('should report when there is no assistant message to recall', async () => {
    const session = createSessionContext();
    const ctxMessages: ContextMessage[] = [
      { role: 'system', content: 'System prompt' },
    ...
    const res = await router.execute('/last');
    expect(res.success).toBe(true);
    expect(res.output).toContain('No assistant messages yet');
  });
});
