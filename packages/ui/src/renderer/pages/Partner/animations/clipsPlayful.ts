...
export function getFakeYawnPose(t: number, actTime: number, baseIdle: VRMPose): VRMPose {
  const tap = Math.sin(actTime * 7.0) * 0.06;
  return {
    ...baseIdle,
    headRot: [-0.1, 0, 0],
    leftUpperArmRot:  [0.08, 0, -1.28],
    ...
    rightLowerArmRot: [0, -2.1 + tap, -0.2],
...
export function getPlayfulPunchPose(t: number, actTime: number, baseIdle: VRMPose): VRMPose {
  ...
  return {
    ...baseIdle,
    spineRot: [0.03 * p, -0.12 * p, 0],
    headRot: [-0.04, 0.08 * p, 0.06],
    leftUpperArmRot:  [0.08, 0, -1.28],
    leftLowerArmRot:  [0, 0.22, 0],
    rightUpperArmRot: [0.55 * p, 0, MathUtils.lerp(1.28, 0.35, p)],
    rightLowerArmRot: [0, -0.85 * p, 0],
    rightHandRot:     [0.15 * p, 0, 0],
    rightFingers:     'fist',
...
export function getMicDropPose(t: number, actTime: number, baseIdle: VRMPose): VRMPose {
  ...
  return {
    ...baseIdle,
    hipsRot: [0, 0.1, 0],
    spineRot: [0.02, -0.06, 0],
    headRot: [-0.05, 0.08, 0.12],
    leftUpperArmRot:  [-0.12, -0.25, -1.12],
    leftLowerArmRot:  [0, 1.35, 0.2],
    leftFingers:      'fist',
    rightUpperArmRot: [0.55, 0, 0.85],
...
export function getDustShouldersPose(t: number, actTime: number, baseIdle: VRMPose): VRMPose {
  const brush = Math.sin(actTime * 9.0) * 0.12;
  return {
    ...baseIdle,
    headRot: [-0.04, -0.15, 0.08],
    leftUpperArmRot:  [0.08, 0, -1.28],
    leftLowerArmRot:  [0, 0.22, 0],
    leftFingers:      'relaxed',
    rightUpperArmRot: [0.55, 0.15, 0.55],
    rightLowerArmRot: [0, -1.85 + brush, -0.2],
    rightHandRot:     [0.2, 0, 0],
    rightFingers:     'open',
...
export function getNekoPose(t: number, actTime: number, baseIdle: VRMPose): VRMPose {
  const pawL = Math.sin(t * 6.5) * 0.15;
  const pawR = Math.cos(t * 6.5) * 0.15;
  const headSway = Math.sin(t * 3.0) * 0.12;

  return {
    ...baseIdle,
    headRot: [0, Math.cos(t * 2.5) * 0.08, headSway],
    leftUpperArmRot:  [0.55, 0.15, -0.85],
    leftLowerArmRot:  [0, 1.85 + pawL, 0.15],
    leftHandRot:      [0.4, 0, 0],
    leftFingers:      'cat',
    rightUpperArmRot: [0.55, -0.15, 0.85],
    rightLowerArmRot: [0, -1.85 - pawR, -0.15],
    rightHandRot:     [0.4, 0, 0],
    rightFingers:     'cat',
