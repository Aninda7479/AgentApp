...
static OLLAMA_STATIC_CACHE: std::sync::Mutex<Option<(bool, Option<String>, Option<String>)>> =
...
pub fn detect_ollama_installation() -> serde_json::Value {
    let (installed, path, version) = {
        let mut lock = OLLAMA_STATIC_CACHE
            .lock()
        ...
        if entry.file_type().is_file() {
            ...
            if let Ok(rel) = path.strip_prefix(&manifests_dir) {
                ...
                if comps.len() >= 2 {
                    ...
                    let model_name = if comps.len() == 4
                        && comps[0] == "registry.ollama.ai"
                        && comps[1] == "library"
                    {
                        format!("{}:{}", comps[2], tag)
                    ...
                    if let Ok(content) = std::fs::read_to_string(path) {
                        if let Ok(json) = serde_json::from_str::<serde_json::Value>(&content) {
                            if let Some(layers) = json.get("layers").and_then(|l| l.as_array()) {
                                for layer in layers {
                                    total_size +=
                                        layer.get("size").and_then(|s| s.as_u64()).unwrap_or(0);
                                }
                            }
                                ...
                                .and_then(|d| d.as_str())
                            {
                                ...
                                if blob_path.exists() {
                                    if let Ok(blob_content) = std::fs::read_to_string(&blob_path) {
                                        if let Ok(blob_json) =
                                            serde_json::from_str::<serde_json::Value>(&blob_content)
                                        {
                                            param_size = blob_json
                                                .get("model_type")
                                                .and_then(|v| v.as_str())
                                                .map(|s| s.to_string());
                                            quant_level = blob_json
                                                .get("file_type")
                                                .and_then(|v| v.as_str())
                                                .map(|s| s.to_string());
                                            family = blob_json
...
static GPU_CACHE: std::sync::Mutex<Option<(std::time::Instant, Vec<serde_json::Value>)>> =
    std::sync::Mutex::new(None);
...
static NPU_CACHE: std::sync::Mutex<Option<(std::time::Instant, serde_json::Value)>> =
    std::sync::Mutex::new(None);
...
pub fn detect_npu_tpu(cpu_brand: &str) -> serde_json::Value {
    ...
    if lower.contains("ryzen ai")
        || lower.contains("xdna")
        || lower.contains("hx 370")
        || lower.contains("hx 365")
        || lower.contains("ai 9")
        || lower.contains("ai 7")
        || lower.contains("ai 5")
    ...
    if lower.contains("snapdragon")
        || lower.contains("x elite")
        || lower.contains("x plus")
...
static SYSTEM_INFO_CACHE: std::sync::Mutex<Option<(std::time::Instant, serde_json::Value)>> =
    std::sync::Mutex::new(None);
...
pub fn get_full_system_info_value() -> serde_json::Value {
    ...
    let cpu_brand = cpus
        ...
        .unwrap_or_else(|| "System CPU".to_string());
    ...
    let is_unified_memory = cfg!(target_os = "macos")
    ...
    let vram_budget_gb = if is_unified_memory {
        ...
        let max_vram = gpus
            .iter()
            .filter_map(|g| g.get("vramGB").and_then(|v| v.as_f64()))
    ...
    if let Some(host) = parsed_url.host_str() {
        if is_private_or_loopback_host(host) {
            return Err((
                ...
                Json(
                    serde_json::json!({ "error": "Access to private or link-local address is denied" }),
                ),
            ));
        }
    }
