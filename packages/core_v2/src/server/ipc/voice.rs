...
pub async fn handle_voice_channel(
    ...
    match ch {
        "voice-transcribe" | "media-transcribe" | "dictation-transcribe" => {
            ...
            let audio_bytes_opt: Option<Vec<u8>> = if let Some(b64) = arg
                .get("audio_base64")
            ...
            let audio_bytes = match audio_bytes_opt {
                ...
                _ => {
                    return Some(Err((
                        ...
                        Json(
                            serde_json::json!({ "error": "No audio data supplied in request", "ok": false }),
                        ),
                    )));
                }
            };
            ...
            .await
            {
                ...
                Err(err) => {
                    ...
                    error!("Voice transcription failed: {}", err_msg);
                    Some(Err((
                        StatusCode::BAD_GATEWAY,
                        Json(serde_json::json!({
                            "ok": false,
                            "error": err_msg,
                            "data": { "text": "", "ok": false, "error": err_msg }
                        })),
                    )))
                }
            }
...
/// Reusable speech-to-text transcription function across Groq Whisper, OpenAI Whisper, and Gemini STT.
pub async fn transcribe_audio_bytes(
    ...
    let raw_settings = settings_store
        .load_raw()
        .unwrap_or_else(|_| serde_json::json!({}));

    // 1. Discover available STT keys (Groq > OpenAI > Gemini)
            ...
            if let Some(k) = key {
                if !k.is_empty() {
                    if id == "groq" && groq_key.is_none() {
                        ...
                        openai_key = Some(k);
                    } else if (id == "gemini" || id == "google" || id.contains("gemini"))
                        && gemini_key.is_none()
                    {
                        gemini_key = Some(k);
                    }
    ...
    if openai_key.is_none() {
        openai_key = settings_store
            .get_api_key("openai")
            .ok()
            .flatten()
            .or_else(|| settings_store.get_api_key("open_ai").ok().flatten())
            .or_else(|| std::env::var("OPENAI_API_KEY").ok());
    }
    if gemini_key.is_none() {
        gemini_key = settings_store
            .get_api_key("gemini")
            .ok()
            .flatten()
            .or_else(|| settings_store.get_api_key("google").ok().flatten())
            .or_else(|| settings_store.get_api_key("google-gemini").ok().flatten())
            .or_else(|| {
    ...
    // 2A. Groq Whisper (~200ms latency)
    if let Some(ref key) = groq_key {
        let part = Part::bytes(audio_bytes.clone())
            .file_name(filename.to_string())
            .mime_str("audio/wav")
            .unwrap_or_else(|_| Part::bytes(audio_bytes.clone()).file_name(filename.to_string()));
    ...
    if transcribed_text.is_none() {
        if let Some(ref key) = openai_key {
            let part = Part::bytes(audio_bytes.clone())
                ...
                .mime_str("audio/wav")
                .unwrap_or_else(|_| {
                    Part::bytes(audio_bytes.clone()).file_name(filename.to_string())
                });
